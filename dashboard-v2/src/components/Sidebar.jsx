  ...
  return (
    <nav className={`bg-surface-container dark:bg-surface-container flex flex-col h-screen fixed left-0 top-0 z-50 docked full-height w-64 border-r border-outline-variant flat no shadows transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="p-container-padding flex items-center gap-3 border-b border-outline-variant/30 h-16 shrink-0">
        <Link to="/" onClick={close} aria-label="Go to homepage" className="shrink-0 rounded-full">
          <BrandMark />
        </Link>
      ...
      <div className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        <NavItem to="/dashboard" icon="dashboard" label="Dashboard" end={true} onNavigate={close} />
        <NavItem to="/dashboard/map" icon="directions_boat" label="Traffic" onNavigate={close} />
        <NavItem to="/dashboard/schedule" icon="schedule" label="Schedule" onNavigate={close} />
        <NavItem to="/dashboard/berth" icon="calendar_view_day" label="Berthing" onNavigate={close} />
        <NavItem to="/dashboard/insights" icon="analytics" label="Insights" onNavigate={close} />
        <NavItem to="/dashboard/predictive" icon="query_stats" label="Predictive" onNavigate={close} />
      </div>
      <div className="p-4 border-t border-outline-variant/30 space-y-1">
        ...
        <NavLink to="/docs" onClick={close} className={({ isActive }) => `text-on-surface-variant hover:text-on-surface px-4 py-3 flex items-center gap-3 hover:bg-surface-variant transition-colors duration-200 rounded-lg mx-2 my-1 group ${isActive ? 'bg-secondary-container text-on-secondary-container active:scale-95' : ''}`}>
