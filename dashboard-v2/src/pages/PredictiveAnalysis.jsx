...
export default function PredictiveAnalysis() {
  const { port, outlookLabels, metadata } = useData();

  if (!port) return null;

  const { metrics, vessels = [], schedule = [], forecast = [] } = port;
  const risk = deriveRisk(metrics.congestionPct);
  const fluidityConfidence = 100 - metrics.congestionPct;
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="flex flex-col gap-2 rounded-xl p-6 bg-surface-container border border-outline-variant shadow-sm">
          <p className="text-on-surface-variant font-body-md font-medium">Fluidity Confidence</p>
          <p className="tracking-tight text-3xl font-bold" style={{ color: RISK_COLOR[risk] }}>{fluidityConfidence}%</p>
          <p className="font-body-sm text-on-surface-variant">derived from congestion score</p>
        </div>
        <div className="flex flex-col gap-2 rounded-xl p-6 bg-surface-container border border-outline-variant shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-surface-container rounded-xl border border-outline-variant p-6 flex flex-col gap-4 shadow-sm">
          <h3 className="text-title-lg text-on-surface font-bold">5-Day Congestion Outlook</h3>
          {forecast.length === 0 ? (
            <div className="flex-1 flex items-center justify-center text-on-surface-variant font-body-sm">No forecast in export</div>
          ) : (
            <>
        ...
        <section className="bg-surface-container rounded-xl border border-outline-variant p-6 flex flex-col gap-3 shadow-sm lg:col-span-2">
          ...
          <div className="flex flex-col gap-2 overflow-y-auto max-h-[240px]">
            {schedule.length === 0 ? (
              ...
              // conf comes from vessel list; find matching or default
              const vessel = vessels.find(vv => vv.mmsi === v.mmsi);
              const conf = vessel?.conf ?? 75;
              return (
                <div key={i} className="flex items-center justify-between py-2 border-b border-outline-variant/30 last:border-0">
                  <div className="flex flex-col">
                    ...
                    <span className="font-data-mono text-[10px] text-on-surface-variant">{v.distance_nm} nm away</span>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="font-data-mono text-[11px] text-primary">{v.eta}</span>
                    <div className="flex items-center gap-1">
                      <span className="font-data-mono text-[10px] text-on-surface-variant">{conf}%</span>
                      <div className="w-12 h-1.5 bg-surface-variant rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full"
                          style={{ width: `${conf}%`, backgroundColor: conf >= 80 ? '#2dd96f' : conf >= 65 ? '#f7b23b' : '#ef4444' }}
                        />
                      </div>
                    </div>
                  </div>
